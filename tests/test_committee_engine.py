...
def test_front_gate_conflict():
    ...
    glm_md = ModelDecision(model_name="qwen", bias="short", confidence=0.8)
    committee = decide_front_gate_sync(
        "BTCUSDm",
        {"features": {"price_30m": 100}},
        overrides={"gpt-4o-mini": gpt_md, "qwen": glm_md},
    )
...
def test_front_gate_low_confidence():
    ...
    glm_md = ModelDecision(model_name="qwen", bias="long", confidence=0.6)
    committee = decide_front_gate_sync(
        "BTCUSDm",
        {"features": {"price_30m": 100}},
        overrides={"gpt-4o-mini": gpt_md, "qwen": glm_md},
...
def test_front_gate_persistence():
    ...
    gpt_md = ModelDecision(model_name="gpt-4o-mini", bias="long", confidence=0.8)
    glm_md = ModelDecision(model_name="qwen", bias="long", confidence=0.75)
    committee = decide_front_gate_sync(
        "BTCUSDm",
        {"features": {"price_30m": 100}},
