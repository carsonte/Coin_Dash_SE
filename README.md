...
- 回测通知：新增开关 `notifications.backtest_enabled`，可关闭回测期间的飞书卡片推送。
...
- 风控开关：`risk.validation_enabled` 启用本地风控校验；`risk.rr_bounds.min` 控制最低 RR。
- CLI 示例
...
- 一键脚本：`scripts/check_release.sh` / `scripts/check_release.ps1`；设置 `RUN_LLM_SMOKE=1` 开启 LLM 冒烟，`RUN_BACKTEST=1` 开启回测示例，`RUN_DEEPSEEK=1` 在回测中启用 DeepSeek
...
Testing status
--------------
- 当前 21 项测试全部通过：`python -m pytest --disable-warnings`（LLM 冒烟在缺 Key 时自动跳过）
