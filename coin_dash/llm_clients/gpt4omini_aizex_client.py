...
def _sync_post(url: str, headers: Dict[str, str], payload: Dict[str, Any], timeout: float) -> Dict[str, Any]:
    ...
    try:
        ...
        raise LLMClientError("invalid JSON response from Aizex") from exc


async def call_gpt4omini(messages: List[Dict[str, Any]], **kwargs: Any) -> Dict[str, Any]:
    """
    调用 Aizex 的 gpt-4o-mini（OpenAI 兼容接口）。
    - 读取环境变量 AIZEX_API_KEY / AIZEX_API_BASE
    ...
    if not base:
        raise LLMClientError("AIZEX_API_BASE is missing")
